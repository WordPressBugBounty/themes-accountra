...
class Init {
	...
	public function template_path( $template_file, $theme_slug, $template_slug ) {
		switch ( $template_slug ) {
			...
			case 'cases':
				return $this->change_stylesheet_directory() . '/templates/cases.html';
			case 'contact':
	...
	public function notice_install_plugin() {
		...
		$button_text = __( 'Install Required Plugins', 'accountra' );
		...
		<style>
			.install-gutenverse-plugin-notice {
				...
				background: url( <?php echo esc_url( ACCOUNTRA_URI . '/assets/img/background-banner.png' ); ?> );
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					...
					nonce: '<?php echo esc_html( wp_create_nonce( 'accountra_admin_notice' ) ); ?>',
				} );
			} );
			...
			<div class="gutenverse-notice-inner">
				<div class="gutenverse-notice-content">
					<div class="gutenverse-notice-text">
						<h3><?php esc_html_e( 'Take Your Website To New Height with', 'accountra' ); ?> <span>Gutenverse!</span></h3> 
						<p><?php esc_html_e( 'Accountra theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Accountra templates built with Gutenverse and get access to more than 40 free blocks, hundred free Layout and Section.', 'accountra' ); ?></p>
					...
					<div class="gutenverse-notice-image">
						<img src="<?php echo esc_url( ACCOUNTRA_URI . '/assets/img/banner-install-gutenverse-2.png' ); ?>"/>
					</div>
				</div>
			</div>
		</div>
	...
	public function backend_scripts() {
		...
		if ( $screen->id === 'appearance_page_accountra-dashboard' ) {
			...
			wp_localize_script( 'accountra-dashboard', 'GutenverseThemeConfig', $this->theme_config() );
		}
	}
	...
	public function theme_config() {
		return array(
			'demo'    => esc_url( 'https:/gutenverse.com/demo?name=accountra' ),
			'pages'   => array(
