...
class Init {
	...
	public function maybe_sync_global_styles_after_version_change() {
		$synced_version = get_option( 'accountra_global_styles_synced_version' );

		if ( ACCOUNTRA_VERSION === $synced_version ) {
			return;
		}
	...
	private function sync_global_styles() {
		...
		update_option( 'accountra_global_styles_synced_version', ACCOUNTRA_VERSION );
	}
	...
	public function setup_theme() {
		load_theme_textdomain( 'accountra', get_template_directory() . '/languages' );
	}
	...
	public function change_stylesheet_directory() {
		return ACCOUNTRA_DIR . 'gutenverse-files/';
	}
	...
	public function notice_closed() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'accountra_admin_notice' ) ) {
  ...
  array (
	...
	public function dashboard_scripts( $hook_suffix ) {
					...
					if ( 'appearance_page_accountra-dashboard' !== $hook_suffix && 'admin_page_gutenverse-onboarding-wizard' !== $hook_suffix ) {
						return;
					}
		...
		$dashboard_includes = include get_template_directory() . '/assets/dependencies/theme-dashboard.asset.php';
					...
					wp_enqueue_style(
						'accountra-dashboard-inter-font',
	...
	public function theme_config() {
		...
		$config = array(
			...
			'version'       => ACCOUNTRA_VERSION,
			'images'        => get_template_directory_uri() . '/assets/img/',
			'title'         => esc_html__( 'Accountra', 'accountra' ),
			'description'   => esc_html__( 'Accountra is an Accounting Firm WordPress Block Theme designed for businesses and professionals who want to build a clean and trustworthy financial website. Built with full site editing and powered by Gutenverse, this theme provides a flexible and modern platform to showcase accounting services, financial expertise, and business solutions with a professional and well-structured layout. Whether you run an accounting agency or provide independent consulting services, Accountra helps you present your expertise with clarity and credibility. It is ideal for accountants, tax consultants, bookkeeping services, financial advisors, and business consulting firms looking to establish a strong online presence. With responsive layouts, customizable block patterns, and organized page structures, you can easily highlight services, company profiles, client testimonials, and case studies. Optimized for performance and usability, Accountra enables you to create a reliable website that reflects your brand identity and expertise in Accounting Firm services. If you are looking for a scalable and professional solution, Accountra is a dependable Accounting Firm WordPress theme to grow your financial business online.', 'accountra' ),
			'pluginTitle'   => esc_html__( 'Plugin Requirement', 'accountra' ),
			'pluginDesc'    => esc_html__( 'This theme require some plugins. Please make sure all the plugin below are installed and activated.', 'accountra' ),
			'note'          => '',
			'note2'         => '',
			'demo'          => '',
			...
			'dashboardPage' => admin_url( 'themes.php?page=accountra-dashboard' ),
			'logo'          => trailingslashit( get_template_directory_uri() ) . 'assets/img/logo-accountra-2.webp',
			'slug'          => 'accountra',
			'upgradePro'    => esc_url( 'https://gutenverse.com/pricing' ),
			'supportLink'   => esc_url( 'https://wordpress.org/support/theme/accountra/ ' ),
...
'lite_page_count' => 0,
'plus_page_count' => 8,
'plus_pattern_count' => 44,
'lite_pattern_count' => 38
			),
			'lite_plus_type' => 'wporg',
			'pro_preview' => trailingslashit( get_template_directory_uri() ) . 'assets/img/accountra-home-pro.webp',
			'pro_title' => esc_html__('Accountra Pro', 'accountra'),
			'upgrade_required_license' => array('professional','agency','enterprise','ultimate'),
		);

		if ( 'themes.php' === $pagenow && isset( $_GET['page'] ) && 'accountra-dashboard' === sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			$admin_config = array(
		...
		return $config;
						...
						public function system_status() {
							...
							$status['active_companion_demo'] = $active_demo['active_demo'] ?? esc_html__( 'You don\'t have any demo activated', 'accountra' );
						...
						public function data_active_plugin() {
							...
							if ( count( $plugins ) > 0 ) {
								foreach ( $plugins as $plugin ) {
									...
									$item['name']        = isset( $plugin['Name'] ) ? $plugin['Name'] : esc_html__( 'unknown', 'accountra' );
									$item['author_uri']  = isset( $plugin['AuthorURI'] ) ? esc_url( $plugin['AuthorURI'] ) : '#';
									$item['author_name'] = isset( $plugin['Author'] ) ? $plugin['Author'] : esc_html__( 'unknown', 'accountra' );
									$item['version']     = isset( $plugin['Version'] ) ? $plugin['Version'] : esc_html__( 'unknown', 'accountra' );

									$content = esc_html__( 'by', 'accountra' );
						...
						public function admin_menu() {
							add_theme_page(
								esc_html__('Accountra Dashboard', 'accountra'),
								esc_html__('Accountra Dashboard', 'accountra'),
								'edit_theme_options',
								'accountra-dashboard',
