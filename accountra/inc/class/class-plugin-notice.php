...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_accountra_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'accountra_dismiss_notice' ) ) {
			return;
		}

		$screen = get_current_screen();
		if ( isset( $screen->parent_file ) && 'themes.php' === $screen->parent_file && 'appearance_page_accountra-dashboard' === $screen->id ) {
			return;
		}
  ...
  '1x' => 'https://ps.w.org/gutenverse-form/assets/icon-128x128.png?rev=3135966',
		...
		wp_register_style( 'accountra-theme-notice', false );
		wp_enqueue_style( 'accountra-theme-notice' );
		ob_start();
		?>
		
				.accountra-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.accountra-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.accountra-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.accountra-simple-notice .accountra-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.accountra-simple-notice .accountra-notice-description {
					font-size: 13px;
					display: block;
				...
				.accountra-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.accountra-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.accountra-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.accountra-simple-notice .notice-dismiss:hover:before,
				.accountra-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'accountra-theme-notice', $custom_style );
		wp_register_script(
			'accountra-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'accountra-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? ACCOUNTRAPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.accountra-simple-notice');
				...
				setTimeout(() => {
					...
					if (dismissBtn) {
						dismissBtn.addEventListener('click', (e) => {
							e.preventDefault();
							jQuery.post(ajaxurl, {
								action: 'accountra_dismiss_notice',
								_ajax_nonce: nonce
							});
						});
					}
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'accountra' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'accountra' ); ?>`;
		...
		wp_add_inline_script( 'accountra-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible accountra-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "accountra_dismiss" ) ); ?>">
				<p>
					<strong class="accountra-notice-title"><?php esc_html_e( "Thank you For Installing Accountra Theme", "accountra" ); ?></strong>
					<span class="accountra-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "accountra" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "accountra" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "accountra" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "accountra" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'accountra_dismiss' );

		if ( ! get_option( 'accountra_dismiss_notice' ) ) {
			update_option( 'accountra_dismiss_notice', true );
		}
